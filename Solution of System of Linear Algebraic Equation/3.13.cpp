/*WAP to find Solution of System of Linear Algebraic Equation with Jacobi’s Inverse Method.*/
#include <iostream>
#include <cmath>
#include <iomanip>
#include <cstring>
...
using namespace std;
double a[3][4];
double x,n_m,y,n_n,z,n_o,err;
int k,counter,error;
void get_matrix()
{
    int i,j;
    cout<<"\n\t\tJACOBIS INVERSE METHOD";
...
void jacobi_inverse(double & m,double & n,double & o)
{
  ...
  n_m = (a[0][3]-a[0][1]*n-a[0][2]*o)/a[0][0];
  n_n = (a[1][3]-a[1][0]*m-a[1][2]*o)/a[1][1];
  n_o = (a[2][3]-a[2][0]*m-a[2][1]*n)/a[2][2];
  err = (abs(m-n_m)+abs(n-n_n)+abs(o-n_o))/3;
